...
res=`docker run -p 9000:9000 --name  php-fpm  -v ${nginx_web_dir}:/var/www/html -v ${docker_php_dir}/logs:/var/log/php  -v ${docker_php_dir}/etc:/usr/local/etc  -d ${docker_name}`
if [ -n "$res" ]; then
