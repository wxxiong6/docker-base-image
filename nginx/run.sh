...
docker_nginx_conf_dir=${docker_nginx_dir}/conf
...
cp -rf ./resources/conf/* ${docker_nginx_conf_dir}/
...
res=`docker run --name nginx  --link php-fpm:php-fpm -p 80:80 -p 443:443 -v ${nginx_web_dir}:/var/www/html -v ${docker_nginx_conf_dir}:/etc/nginx/conf -v ${docker_nginx_dir}/logs:/var/log/nginx -d ${docker_name}`
if [ -n "$res" ]; then
